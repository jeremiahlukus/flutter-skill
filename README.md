...
with `--json`, and is equally usable on its own — it is maintained separately at
[jeremiahlukus/android-env](https://github.com/jeremiahlukus/android-env).
...
<details>
<summary><strong>android-env — full CLI reference</strong></summary>

You never need to run this directly; the MCP tools above cover the common
cases. It is documented here because it is useful on its own, in a CI job, or
when you want to see exactly what the tools are doing.

A single-file Bash CLI that sets up a complete Android development environment
without installing Android Studio: the SDK command-line tools, platform tools, a
system image and an emulator, plus a virtual device with a realistic phone frame
and a desktop launcher. Works on macOS, Linux and Windows (via Git Bash).

#### Using it directly

From this repo:

    ./scripts/android-env doctor
    ./scripts/android-env ensure --json

Or put it on your `PATH` so it works from any directory:

    ./scripts/android-env install
    android-env ensure --json

`install` picks the first directory that is both on your `PATH` and writable
without `sudo`, trying `~/.local/bin`, then `~/bin`, then `/usr/local/bin`. It
symlinks, so later edits take effect immediately; where symlinks are restricted
it installs a copy and says so. Pass a directory to choose yourself
(`install ~/bin`), and if it is not on your `PATH` the exact `export` line is
printed. `uninstall` removes only the `PATH` entry — your SDK, emulators and
...
`ensure` is the one call to make from a script, a CI job or an MCP server:
...
`{"success": false, "error": "..."}` rather than a bare non-zero exit, so a
...
into the tool cannot hang it. The MCP tools rely on this.
...
You are shown what will be removed and its size, and must retype the name to
confirm. Add `-y` to skip the prompt. Deleting is refused while that emulator is
running, and never touches the SDK, system images or your other emulators.

#### Configuration

Nothing needs configuring. These exist for pinning a version, forcing an
upgrade, or working offline.
...
committing to it. To upgrade an existing install:
...
**Modern installer.** SDK packages are installed with the `android` CLI that
ships with command-line tools revision 23 and later, passing `--no-metrics` to
opt out of usage-data collection. Older tools fall back to `sdkmanager`.
...
**No interference.** `sdkmanager`, `avdmanager`, `emulator` and `adb` are called
by absolute path, so another Android SDK earlier in your `PATH`, from Homebrew
or Android Studio, cannot intercept the install. `doctor` warns you when it
finds one.

**Architecture detection.** Picks `arm64-v8a` on Apple Silicon and ARM Linux,
`x86_64` elsewhere.

**Device frames.** Downloads Pixel 6 Pro, Pixel 6 and Pixel 5 frames and
configures your emulator with the one you choose, so it looks like a real phone.
...
path it looked in. `android-env tools` shows whether your command-line tools are
current and how to upgrade them.
...
command-line tools are older than the rest of your SDK:

    ANDROID_CMDLINE_TOOLS_UPGRADE=1 android-env setup

*"The SDK Manager CLI tool (sdkmanager) is deprecated"* is expected on
command-line tools revision 23 and later. The script calls the `android` CLI
directly, so you will only see this with `ANDROID_ENV_USE_SDKMANAGER=1`.

**Reclaiming disk space.** Emulators are large, often several GB each.
`android-env delete` removes one along with its launcher. To also remove the
